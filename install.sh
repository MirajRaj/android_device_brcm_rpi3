...
#
# Original author of this script: Igor Kalkov (ikalkov) - https://github.com/RTAndroid/android_device_brcm_rpi3/blob/aosp-n/install.sh
# Modified by: udK - https://github.com/udK
#
...
copy_files()
{
    BOOT_DIR="./device/brcm/rpi3/boot"
    if [ ! -d $BOOT_DIR ]; then
    ...
    SYSTEM_IMG="./out/target/product/rpi3/system.img"
    if [ ! -f $SYSTEM_IMG ]; then
        echo "ERR: system image not found!"
        exit 1
    fi

    RAMDISK_IMG="./out/target/product/rpi3/ramdisk.img"
    if [ ! -f $RAMDISK_IMG ]; then
        echo "ERR: ramdisk image not found!"
        exit 1
    fi
